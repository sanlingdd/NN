...
for i in range(enhanceIter):
    ...
    for columnName in dfless.iloc[0].index:
        if not columnName.startswith('Next'):
            ...
            dflessCopy[columnName] = dflessCopy[columnName].reshape(len(dfless),1) + normalarr
    df = df.append(dflessCopy)
cursor.close()
conn.close()
...
y_train = nd.array(all_X.NextDayIncrease3[:num_train].as_matrix())
y_test = nd.array(all_X.NextDayIncrease3[num_train:].as_matrix())
...
x_predict = nd.array(Predict.loc[:,'Day1OpenPrice':'Week4Exchange'].as_matrix())
y_predict = nd.array(Predict.NextDayIncrease3[num_train:].as_matrix())
...
batch_size = 16
utils.trainXY(X_train,y_train, X_test,y_test,x_predict,y_predict, batch_size, net, loss,trainer, ctx, num_epochs=10)
